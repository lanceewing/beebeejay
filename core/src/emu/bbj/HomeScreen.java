...
import emu.bbj.config.AppConfig;
...
import emu.bbj.ui.PagedScrollPane;
import emu.bbj.ui.TextInputResponseHandler;
import emu.bbj.ui.ViewportManager;

/**
 * The Home screen of the BeeBeeJay emulator, i.e. the one that shows all of the 
...
public class HomeScreen extends InputAdapter implements Screen  {
   ...
   * @param dialogHandler
   */
  public HomeScreen(BeeBeeJay bbj, DialogHandler dialogHandler) {
    this.bbj = bbj;
    ...
    bbj.getPreferences().putString("home_screen_app_list", appConfigJson);
    ...
    loadScreenShotProperties(Gdx.files.internal("data/beebeejay_screens.store"));
  ...
  private void addAppButtonsToStage(Stage stage, AppConfig appConfig, int columns, int rows) {
    ...
    Texture titleTexture = new Texture("png/joric_title.png");
    ...
    if (keycode == Keys.BACK) {
      if (Gdx.app.getType().equals(ApplicationType.Android)) {
        dialogHandler.confirm("Do you really want to Exit?", new ConfirmResponseHandler() {
          public void yes() {
            // Pressing BACK from the home screen will leave BeeBeeJay.
            Gdx.app.exit();
          }
          public void no() {
  ...
  public Button buildAppButton(AppConfigItem appConfigItem) {
    ...
    if ((appConfigItem.getIconPath() != null) && (!appConfigItem.getIconPath().equals(""))) {
      ...
      if (screenshotData == "") {
        screenshotData = bbj.getScreenshotStore().getString(friendlyAppName, "");
      }
      if (screenshotData != "") {
        try {
      ...
      if ((appName != null) && (!appName.equals(""))) {
        ...
        if (appConfigItem != null) {
          if (longPressActor != null) {
            ...
            MachineScreen machineScreen = bbj.getMachineScreen();
            machineScreen.initMachine(appConfigItem);
            bbj.setScreen(machineScreen);
          }
        }
      } else {
        String startPath = bbj.getPreferences().getString("open_app_start_path", null);
        dialogHandler.openFileDialog("", startPath, new OpenFileResponseHandler() {
          ...
          public void openFileResult(boolean success, final String filePath) {
            if (success && (filePath != null) && (!filePath.isEmpty())) {
              ...
              bbj.getPreferences().putString("open_app_start_path", fileHandle.parent().path());
              bbj.getPreferences().flush();
