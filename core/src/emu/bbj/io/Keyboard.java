...
/**
 * This class emulates the BBC keyboard by listening to key events, translating
 * them in to BBC "key codes", and then responding to BBC keyboard scans when they 
 * are invoked.
 * 
 * @author Lance Ewing
 */
public class Keyboard {
  ...
  /**
   * Data used to convert Java keypresses into BBC keypresses.
   */
  ...
  /**
   * HashMap used to store mappings between Java key events and BBC
   * keyboard scan codes.
   */
  ...
  public void keyPressed(int keycode) {
    ...
    // Update the key matrix to indicate to the BBC that this key is down.
  ...
  /**
   * Invoked when a key is typed. This method is used as a workaround for several keys
   * that are not processed properly by the keyPressed and keyReleased events.
   * 
  ...
  public void keyReleased(int keycode) {
    ...
    if (currentTime < minKeyReleaseTime) {
      ...
      // Otherwise we process the release by updating the key matrix that the BBC polls.
      int keyDetails[] = (int[])keyConvHashMap.get(new Integer(keycode));
      if (keyDetails != null) {
        keyMatrix[keyDetails[1]] &= (~keyDetails[2]);
      } else {
  ...
  public boolean isKeyPressed(int selectedRow) {
    int selectedColumns = 0xFF;
    
    boolean keypressed = (keyMatrix[selectedRow] & selectedColumns) != 0;
