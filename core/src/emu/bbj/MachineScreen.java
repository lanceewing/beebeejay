...
import emu.bbj.ui.MachineInputProcessor;
import emu.bbj.ui.ViewportManager;

/**
 * The main screen in the BeeBeeJay emulator, i.e. the one that shows the video 
 * output of the ULA.
 * 
 * @author Lance Ewing
 */
public class MachineScreen implements Screen {
  ...
  /**
   * The Game object for BeeBeeJay. Allows us to easily change screens.
   */
  private BeeBeeJay bbj;
  
  /**
   * This represents the BBC machine.
   */
  ...
  // Currently in use components to support rendering of the BBC screen. The objects 
   ...
   * 
   * @param bbj The BeeBeeJay instance.
   * @param dialogHandler
   */
  public MachineScreen(BeeBeeJay bbj, DialogHandler dialogHandler) {
    this.bbj = bbj;
   ...
   * Textures, Camera and Viewport required to render the BBC screen at the size needed for
  ...
  public void initMachine(AppConfigItem appConfigItem) {
    ...
    // Switch libGDX screen resources used by the BBC screen to the size required by the MachineType.
  ...
  private void createScreenResourcesForMachineType(MachineType machineType) {
    // Create the libGDX screen resources used by the BBC screen to the size required by the MachineType.
  ...
  private void draw(float delta) {
    ...
    if (keyboardType.isRendered()) {
      ...
      if (viewportManager.isPortrait()) {
        ...
        if (Gdx.app.getType().equals(ApplicationType.Android)) {
          // Main BBC keyboard on the right.
          ...
          // Desktop puts BBC keyboard button in the middle.
          batch.draw(keyboardIcon, viewportManager.getWidth() - viewportManager.getWidth()/2 - 70, 0);
    ...
    float bbcScreenHeight = (viewportManager.getHeight() - (viewportManager.getWidth()  / 5) * 4);
    if (machineRunnable.isWarpSpeed()) {
      batch.setColor(0.5f, 1.0f, 0.5f, 1.0f);
    } else {
      batch.setColor(c.r, c.g, c.b, 0.5f);
    }
    batch.draw(warpSpeedIcon, 0, bbcScreenHeight - 128);
    batch.setColor(c.r, c.g, c.b, 0.5f);
    batch.draw(cameraIcon, viewportManager.getWidth() - 145, bbcScreenHeight - 128);
    if (showFPS) {
      font.draw(batch, String.format("%4dFPS", machineRunnable.getFramesLastSecond()), 
          viewportManager.getWidth() - viewportManager.getWidth()/2 - 90, bbcScreenHeight - 48);
    }
    batch.end();
    if (keyboardType.equals(KeyboardType.JOYSTICK)) {
      float joyX = 0;
  ...
  public void saveScreenshot() {
    ...
    if (Gdx.app.getType().equals(ApplicationType.Desktop)) {
      try {
        StringBuilder filePath = new StringBuilder("beebeejay_screens/");
    ...
    if (appConfigItem != null) {
      try {
        ...
        bbj.getScreenshotStore().putString(friendlyAppName, new String(Base64Coder.encode(out.toByteArray())));
        bbj.getScreenshotStore().flush();
  ...
  public void resize(int width, int height) {
    ...
    // Align BBC screen's top edge to top of the viewport.
   ...
   */
  public void exit() {
    bbj.setScreen(bbj.getHomeScreen());
  }
}
